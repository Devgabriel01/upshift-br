...
export default function AdminBlog() {
  ...
  const [form, setForm] = useState({
    ...
    category: "", readTime: "5 min", status: "DRAFT",
  });
  ...
  function handleEdit(post: Post) {
    ...
    setForm({
      ...
      readTime: post.readTime, status: post.status,
    });
    ...
    <main style={{ minHeight: "100vh", background: "#0f0f10", color: "#fff", padding: "40px 4%" }}>
      <div style={{ maxWidth: "900px", margin: "0 auto" }}>
        ...
        <p style={{ color: "rgba(255,255,255,0.4)", marginBottom: "32px", fontSize: "14px" }}>
          Painel de gerenciamento do blog
        </p>
        ...
        <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "16px", padding: "32px", marginBottom: "40px" }}>
          ...
          <div style={{ display: "flex", gap: "12px", alignItems: "center", flexWrap: "wrap" }}>
            ...
            {editing && (
              <button onClick={cancelEdit} style={{ background: "transparent", color: "rgba(255,255,255,0.5)", padding: "10px 20px", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.1)", cursor: "pointer", fontSize: "14px" }}>
                Cancelar
              </button>
            )}
            {msg && (
              <span style={{ color: msg.includes("Erro") ? "#ef4444" : "#10b981", fontSize: "14px" }}>
                {msg}
              </span>
            )}
          </div>
        ...
        <div style={{ display: "grid", gap: "12px" }}>
          {posts.length === 0 && (
            <p style={{ color: "rgba(255,255,255,0.3)", fontSize: "14px" }}>Nenhum artigo ainda.</p>
          )}
          {posts.map(post => (
            <div key={post.id} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "12px", padding: "20px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: "16px" }}>
              <div style={{ flex: 1 }}>
                ...
                <p style={{ fontSize: "13px", color: "rgba(255,255,255,0.4)" }}>
                  {post.category} · {post.readTime} · /{post.slug}
                </p>
              </div>
              <div style={{ display: "flex", gap: "8px" }}>
                <button onClick={() => handleEdit(post)} style={{ background: "rgba(255,107,0,0.1)", color: "#FF6B00", padding: "8px 16px", borderRadius: "8px", border: "1px solid rgba(255,107,0,0.2)", cursor: "pointer", fontSize: "13px", fontWeight: 600 }}>
                  Editar
                </button>
                <button onClick={() => handleDelete(post.id)} style={{ background: "rgba(239,68,68,0.1)", color: "#ef4444", padding: "8px 16px", borderRadius: "8px", border: "1px solid rgba(239,68,68,0.2)", cursor: "pointer", fontSize: "13px", fontWeight: 600 }}>
                  Deletar
                </button>
              </div>
            </div>
